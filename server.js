...
// V6 wallet ownership proof + Telegram webhook verification.
const TON_PROOF_DOMAIN =
  String(process.env.TON_PROOF_DOMAIN || "jaslin-game.vercel.app")
    .replace(/^https?:\/\//i, "")
    .replace(/\/$/, "")
    .trim();
...
const TON_PROOF_MAX_AGE_SECONDS = Math.max(
  ...
  Math.min(
    1800,
    Number(process.env.TON_PROOF_MAX_AGE_SECONDS || 900)
  )
);
...
// Baseline response hardening. CSP remains compatible with Telegram/TON Connect.
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "frame-src https:; object-src 'none'; base-uri 'self'; form-action 'self' https:"
  );

  if (String(req.headers["x-forwarded-proto"] || "https") === "https") {
    res.setHeader(
...
/* =========================================
   DISTRIBUTED API PROTECTION (SUPABASE)
   Works across Vercel instances/cold starts.
========================================= */
...
function createRateLimit({ windowMs, max, prefix }) {
  return async (req, res, next) => {
    try {
      const key = `${prefix}:${clientIdentity(req)}`;
      ...
      if (result.allowed === false) {
        const retryAfter = Math.max(
          1,
          Math.ceil(
            (Number(result.reset_at || Date.now()) - Date.now()) / 1000
          )
        );
      ...
      // Fail closed for financial/reward endpoints. This is safer than
      // silently disabling abuse protection when the limiter store is unavailable.
      console.error("Distributed rate limit error:", safeErrorDetails(error));
      return res.status(503).json({
        error: "Proteksi request sedang tidak tersedia. Coba lagi sebentar."
      });
    }
  };
}
...
const limitSession = createRateLimit({ windowMs: 60_000, max: 30, prefix: "session" });
...
const limitWallet = createRateLimit({ windowMs: 60_000, max: 8, prefix: "wallet" });
const limitWalletProof = createRateLimit({ windowMs: 60_000, max: 8, prefix: "walletproof" });
...
app.get("/api/config", (req, res) => {
  res.json({
    ...
    tonProofDomain: TON_PROOF_DOMAIN
  });
});
    ...
    try {
      ...
      const {
        ...
        await supabase.rpc(
          "jaslin_commit_mining_claim",
          {
            ...
            p_expected_mining_started_at:
              Number(
                user.mining_started_at || 0
              ),
            p_claimed:
              claimed,
            p_referrer:
      ...
      if (!data?.ok) {
        return res.status(409).json({
          error:
            data?.error ||
            "Claim sedang diproses atau saldo berubah. Coba lagi."
        });
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const walletPromise =
        user.wallet && Number(user.wallet_verified_at || 0) > 0
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const gameBalance = Number(user.balance || 0);

      /*
       * SECURITY RULE:
       * Upgrade eligibility still uses a FRESH on-chain wallet balance.
       * We intentionally do not reuse a stale UI quote for authorization.
       */
      let walletBalance = 0;

      if (user.wallet && Number(user.wallet_verified_at || 0) > 0) {
        try {
          walletBalance =
      ...
      /*
       * Atomic-ish optimistic level transition:
       * only succeeds if the stored level is still the level we just checked.
       */
      const { data: upgradedUser, error } =
        await supabase
          ...
          .select("telegram_id,level,balance")
          .maybeSingle();
      ...
      if (!upgradedUser) {
        return res.status(409).json({
          error:
            "Level sudah berubah. Muat ulang lalu coba lagi."
        });
      }

      /*
       * Keep audit history, but do not run publicState() here.
       * publicState() performs extra mining/pulse/referral reads that do not
       * change the authorization result and were making Upgrade feel sluggish.
       */
      await saveTransaction(
        req.tgUser.id,
        "upgrade_core",
      ...
      return res.json({
        ...
        // Balance is not spent by a Core upgrade.
        balance: Number(upgradedUser.balance ?? gameBalance),
        ...
        // Useful for frontend diagnostics/performance monitoring.
...
async function findConfirmedJettonTransfer(withdrawal) {
  if (!withdrawal?.query_id) return null;
...
async function reconcileWithdrawalRow(row) {
  if (
    !row ||
  ...
  try {
    const chain =
      await findConfirmedJettonTransfer(
        row
      );
    ...
    const {
      ...
      await supabase
        ...
        .select(
          "request_id,wallet,amount,fee,receive_amount,status,query_id,tx_hash,error_message,created_at,updated_at"
        )
        .maybeSingle();
    ...
    if (updated) {
      await saveTransaction(
        ...
        `Auto WD ${row.request_id} confirmed on-chain`,
        chain.txHash,
        "confirmed"
      );
    ...
    console.warn(
      "Withdrawal reconciliation warning:",
      safeErrorDetails(error)
    );
  }
...
app.get(
  ...
  async (req, res) => {
    try {
      const { data, error } = await supabase
        .from("withdrawals")
        .select("request_id, telegram_id, wallet, amount, fee, receive_amount, status, query_id, tx_hash, error_message, created_at, updated_at")
        .eq("telegram_id", String(req.tgUser.id))
        .order("created_at", { ascending: false })
        .limit(20);
      ...
      if (candidates.length) {
        ...
        const { data: refreshed, error: refreshError } =
          await supabase
            .from("withdrawals")
            .select("request_id, wallet, amount, fee, receive_amount, status, query_id, tx_hash, error_message, created_at, updated_at")
            .eq("telegram_id", String(req.tgUser.id))
            .order("created_at", { ascending: false })
            .limit(20);
  ...
  async (req, res) => {
    try {
      ...
      if (wallet) {
        return res.status(400).json({
          error:
            "Wallet harus diverifikasi melalui TON Proof. Direct wallet write dinonaktifkan."
        });
      }
      ...
      console.error("Clear wallet error:", safeErrorDetails(error));
      return res.status(500).json({
        error: "Gagal menghapus wallet."
      });
    }
  }
);
...
/* =========================================
   TON PROOF — WALLET OWNERSHIP
========================================= */

function signatureDomainForNetwork(network) {
  if (network === "-239" || network === "-3") {
    return { type: "empty" };
  }

  const globalId = Number(network);
  if (!Number.isInteger(globalId)) {
    throw new Error("Network wallet tidak valid.");
  }
...
function buildTonProofDigest(address, proof) {
  const domainBytes = Buffer.from(
    String(proof?.domain?.value || ""),
    "utf8"
  );

  if (
    Number(proof?.domain?.lengthBytes) !==
    domainBytes.length
  ) {
    throw new Error("TON Proof domain length tidak valid.");
  }
  ...
  const msg = Buffer.concat([
    ...
    domainBytes,
    ts,
    Buffer.from(String(proof.payload || ""), "utf8")
  ]);
...
async function walletPublicKeyFromChain(address) {
  try {
    const result =
      await tonClient.runMethod(
        address,
    ...
    throw new Error(
      "Public key wallet belum bisa diverifikasi on-chain. Pastikan wallet sudah diinisialisasi."
    );
  }
}

async function verifyTonOwnership({
  account,
  proof
}) {
  const addressText =
  ...
  const network =
    String(account?.chain || "").trim();

  const walletStateInit =
    String(account?.walletStateInit || "").trim();

  if (!addressText || !walletStateInit) {
    throw new Error("Data wallet TON Proof tidak lengkap.");
  }

  if (network !== "-239") {
    throw new Error("JASLIN hanya menerima TON mainnet.");
  }
  ...
  try {
    ...
    throw new Error("walletStateInit tidak valid.");
  }
  ...
  if (!derived.equals(address)) {
    throw new Error("walletStateInit tidak cocok dengan address.");
  }
  ...
  if (!expectedDomain || proofDomain !== expectedDomain) {
    throw new Error("TON Proof domain tidak cocok.");
  }

  const timestamp =
    Number(proof?.timestamp || 0);
  ...
  if (
    ...
    throw new Error("TON Proof sudah kedaluwarsa.");
  }

  const payload =
    String(proof?.payload || "");

  if (
    !/^[A-Za-z0-9_-]{32,128}$/.test(payload)
  ) {
    throw new Error("TON Proof payload tidak valid.");
  }

  const { data: nonceRow, error: nonceError } =
    await supabase
      .from("jaslin_ton_proof_nonces")
      .select(
        "telegram_id,payload,expires_at,used_at"
      )
      .eq("telegram_id", String(account.telegramId))
      .eq("payload", payload)
      .maybeSingle();
  ...
  if (
    !nonceRow ||
    Number(nonceRow.used_at || 0) > 0 ||
    Number(nonceRow.expires_at || 0) < Date.now()
  ) {
    throw new Error("TON Proof nonce tidak valid atau sudah digunakan.");
  }

  const publicKey =
    await walletPublicKeyFromChain(address);
  ...
  try {
    ...
    throw new Error("TON Proof signature tidak valid.");
  }
  ...
  const valid =
    domainSignVerify({
      ...
      publicKey,
      domain:
        signatureDomainForNetwork(
          network
        )
    });

  if (!valid) {
    throw new Error("Signature TON Proof tidak valid.");
  }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "TON proof status error:",
        safeErrorDetails(error)
      );

      return res.status(500).json({
        error:
          "Gagal memeriksa status wallet."
      });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Keep only current unconsumed challenges for this Telegram user.
      await supabase
        .from("jaslin_ton_proof_nonces")
        .delete()
        .eq("telegram_id", telegramId)
      ...
      console.error(
        "TON proof payload error:",
        safeErrorDetails(error)
      );

      return res.status(500).json({
        error:
          "Gagal menyiapkan verifikasi wallet."
      });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Atomically consume this exact nonce.
      const consumedAt =
        Date.now();
      ...
      if (!consumed) {
        return res.status(409).json({
          error:
            "TON Proof sudah pernah digunakan. Connect ulang wallet."
        });
      }

      const wallet =
        verified.address.toString({
          bounceable: false,
          urlSafe: true
        });
      ...
      const {
        ...
        await supabase
          ...
          .select(
            "wallet,wallet_verified_at,wallet_chain"
          )
          .single();
      ...
      return res.json({
        ...
        walletVerifiedAt:
          Number(
            updated.wallet_verified_at
          ),
        walletChain:
          updated.wallet_chain
      });
      ...
      return res.status(400).json({
        error:
          error?.message ||
          "Verifikasi kepemilikan wallet gagal."
      });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        !user.wallet ||
        Number(user.wallet_verified_at || 0) <= 0
      ) {
        return res.status(400).json({
          error:
            "Wallet belum terverifikasi. Connect wallet melalui TON Proof terlebih dahulu."
        });
      }
      ...
      const claimed = await patchWithdrawal(
        { status: "pending", query_id: queryId, error_message: null },
        "reserved"
      );
      if (!claimed) {
      ...
      await patchWithdrawal({
        ...
        error_message:
          chain.seqnoAdvanced
            ? null
            : "Broadcast submitted; awaiting on-chain reconciliation"
      });
      ...
      return res.status(broadcastStarted ? 202 : 500).json({
        ...
        requestId,
        withdrawalStatus: broadcastStarted ? "sent" : "refunded"
      });
    } finally {
        ...
        await telegramApi(
          "sendMessage",
          {
            ...
            text:
              ...
              "💎 Wallet — Connect and verify your TON wallet for Holding Power and withdrawals.\n\n" +
